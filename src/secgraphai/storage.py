...
class Storage:
    ...
    def _initialize(self) -> None:
        with self._connect() as connection:
            connection.execute("CREATE TABLE IF NOT EXISTS reports (scan_id TEXT PRIMARY KEY, "
                               "created_at TEXT NOT NULL, document TEXT NOT NULL)")

    def save(self, report: Report) -> None:
        with self._connect() as connection:
            connection.execute("INSERT OR REPLACE INTO reports VALUES (?, ?, ?)",
                               (report.scan_id, report.finished_at.isoformat(), to_json(report)))

    def get(self, scan_id: str) -> Report | None:
        with self._connect() as connection:
            row = connection.execute("SELECT document FROM reports WHERE scan_id = ?", (scan_id,)).fetchone()
