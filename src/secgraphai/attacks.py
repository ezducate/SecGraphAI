...
class AttackMemory:
    ...
    def observe(self, attack: Attack, *, stage: str, score: float,
...
class AttackPack:
    ...
    def payload(self) -> dict[str, Any]:
        return {"id": self.id, "version": self.version, "publisher": self.publisher,
                "minimum_engine": self.minimum_engine,
                "attacks": [{"id": a.id, "family": a.family, "prompt": a.prompt,
                             "cost": a.cost, "tags": sorted(a.tags), "source": a.source,
                             "target_invariant": a.target_invariant,
                             "conversation_depth": a.conversation_depth}
                            for a in self.attacks]}


def load_pack(path: str | Path, *, allow_unverified: bool = False,
              public_key: bytes | None = None, max_bytes: int = 2_000_000) -> AttackPack:
    ...
    for item in raw["attacks"]:
        ...
        attacks.append(Attack(str(item["id"]), str(item["family"]), str(item["prompt"]),
                              int(item.get("cost", 1)), frozenset(item.get("tags", [])),
                              str(item.get("source", "user")), item.get("target_invariant"),
                              int(item.get("conversation_depth", 1))))
    pack = AttackPack(str(raw["id"]), str(raw["version"]), str(raw["publisher"]),
                      str(raw.get("minimum_engine", "1.0.0")), tuple(attacks),
                      PackTrust(raw.get("trust", "LOCAL")), raw.get("signature"))
...
class AttackPlanner:
    ...
    def __init__(self, attacks: Iterable[Attack] = BUILTIN_ATTACKS,
                 memory: AttackMemory | None = None) -> None:
        self.attacks = tuple(attacks)
        self.memory = memory or AttackMemory()

    def plan(self, features: Iterable[str], *, budget: int,
             previous_prompts: Iterable[str] = ()) -> list[Attack]:
        feature_set = {item.casefold() for item in features}
        relevant = [attack for attack in self.attacks if not attack.tags or
                    {tag.casefold() for tag in attack.tags} & feature_set or
                    attack.family.casefold() in feature_set]
        if not relevant:
            relevant = list(self.attacks)
        ranked = sorted(relevant, key=lambda item: (
            -self.memory.scores.get(item.id, 0), -novelty(item.prompt, previous_prompts),
            item.cost, item.id))
