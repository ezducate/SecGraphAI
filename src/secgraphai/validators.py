...
def _evidence(
    kind: str, description: str, observed: str | None = None, **metadata: Any
) -> Evidence:
    payload = json.dumps(
        {"kind": kind, "description": description, "observed": observed, "metadata": metadata},
        sort_keys=True,
        default=str,
    ).encode()
    return Evidence(
...
class Validators:
    ...
    def contains(output: str, marker: str) -> Validation:
        ...
        return Validation(
            Verdict.VERIFIED_VIOLATION if found else Verdict.PASS,
            1.0,
...
class AuthorizationValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        status_failed = bool(
            context.expected_status and context.status_code not in context.expected_status
        )
        marker_failed = any(marker in context.output for marker in self.forbidden_markers)
        failed = status_failed or marker_failed
        return Validation(
            Verdict.VERIFIED_VIOLATION if failed else Verdict.PASS,
            1.0,
            (
                _evidence(
                    "authorization",
                    "Authorization expectation",
                    str(context.status_code),
                    expected=sorted(context.expected_status),
                ),
            ),
        )
...
class StateValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        ...
        return Validation(
            Verdict.VERIFIED_VIOLATION if changed else Verdict.PASS,
            1.0,
...
class ToolInvocationValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        ...
        return Validation(
            Verdict.VERIFIED_VIOLATION if unexpected else Verdict.PASS,
            1.0,
            (
                _evidence(
                    "tool",
                    "Observed tool invocations",
                    ",".join(sorted(tools)),
...
class HTTPCallbackValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        ...
        return Validation(
            Verdict.VERIFIED_VIOLATION if found else Verdict.PASS,
            1.0,
            (_evidence("http-callback", "Controlled callback observation", self.expected_token),),
        )
...
class JSONSchemaValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        try:
            ...
            return Validation(
                Verdict.VERIFIED_VIOLATION, 1.0, (_evidence("json-schema", "Response is not JSON"),)
            )
...
class SecretLeakValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        matched = next(
            (pattern for pattern in self.patterns if re.search(pattern, context.output)), None
        )
        return Validation(
            Verdict.VERIFIED_VIOLATION if matched else Verdict.PASS,
            1.0,
...
class PolicyDecisionValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        ...
        return Validation(
            Verdict.PASS if effect in self.allowed_effects else Verdict.VERIFIED_VIOLATION,
            1.0,
            (_evidence("policy", "Policy effect", effect),),
        )
...
class CostValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        return Validation(
            Verdict.VERIFIED_VIOLATION if context.cost_usd > self.maximum_usd else Verdict.PASS, 1.0
        )
...
class LatencyValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        return Validation(
            Verdict.VERIFIED_VIOLATION if context.latency_ms > self.maximum_ms else Verdict.PASS,
            1.0,
        )
...
class ResponseCodeValidator:
    ...
    def validate(self, context: ValidationContext) -> Validation:
        return Validation(
            Verdict.PASS if context.status_code in self.expected else Verdict.VERIFIED_VIOLATION,
            1.0,
        )
...
async def judge_ensemble(prompt: str, output: str, judges: Iterable[Judge]) -> Validation:
    ...
    return Validation(
        Verdict.LIKELY_VIOLATION if yes * 2 > len(votes) else Verdict.PASS, confidence
    )
