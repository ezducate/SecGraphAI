...
class InvariantEngine:
    ...
    def evaluate_flow(self, source: dict[str, Any], destination: dict[str, Any]) -> list[Evaluation]:
        results = []
        for rule in self.invariants:
            if _matches(rule.source, source) and _matches(rule.destination, destination):
                ...
                reason = "observed flow violates deny invariant" if denied else "observed allowed flow"
                results.append(Evaluation(invariant_id=rule.id, verdict=verdict, reason=reason,
                    evidence=[Evidence(kind="flow", description=reason, metadata={
