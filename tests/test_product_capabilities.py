...
from secgraphai.attacks import BUILTIN_ATTACKS, AttackMemory, evolve, novelty, select_attacks, sign_pack, verify_pack
from secgraphai.config import Config, Mode
from secgraphai.core import Evidence, Finding, Report, Severity, Verdict
from secgraphai.intelligence import Component, Vulnerability, coverage, parse_cyclonedx, security_gate
...
from secgraphai.targets import Identity, audit_mcp_tools, discover_openapi, identity_matrix, schemathesis_cases
from secgraphai.validators import Validators, judge_ensemble


def finding(title="issue", verdict=Verdict.VERIFIED_VIOLATION):
    return Finding(id="SG-1", title=title, severity=Severity.HIGH, verdict=verdict,
                   confidence=1, evidence=[Evidence(kind="x", description="x", observed="MARK")])
...
def test_regex_and_json_validators_matrix(index):
    assert Validators.regex(f"tenant-{index}", rf"tenant-{index}$").verdict == Verdict.VERIFIED_VIOLATION
    payload = json.dumps({"records": [{"owner": index}]})
    assert Validators.json_path(payload, "$.records.0.owner", index).verdict == Verdict.VERIFIED_VIOLATION
...
@pytest.mark.parametrize("score,known,reachable", [(s, k, r) for s in range(1, 11) for k in (False, True) for r in (False, True, None)])
...
def test_openapi_identity_and_schemathesis():
    spec = {"security": [{"bearer": []}], "paths": {"/users": {"get": {"operationId": "users"}},
            "/admin": {"post": {"security": [{"admin": []}]}}}}
    endpoints = discover_openapi(spec)
    assert len(endpoints) == 2 and endpoints[0].security == ("bearer",)
    cases = identity_matrix([Identity("guest"), Identity("root", roles=frozenset({"admin"}))],
                            endpoints, lambda identity, endpoint: "admin" in identity.roles)
...
def test_policy_shadow_simulation_and_explanation():
    engine = PolicyEngine([Rule("deny-shell", Effect.DENY, {"tool": "shell"}, "dangerous")], shadow=True)
    decision = engine.evaluate({"tool": "shell"})
    assert decision.effect == Effect.DENY and not decision.enforced and decision.reason == "dangerous"
...
def test_provenance_merge():
    ...
    assert merged.trust == Trust.MIXED and merged.tenant == "a" and "summarize" in merged.transformations
...
def test_sbom_gate_and_config():
    ...
    assert security_gate([Vulnerability("CVE-1", "x", 9)])[0] is False
    assert Config().mode == Mode.SAFE


@pytest.mark.asyncio
async def test_judge_ensemble_majority_and_tie():
    async def yes(prompt, output): return True, .8
    async def no(prompt, output): return False, .9
